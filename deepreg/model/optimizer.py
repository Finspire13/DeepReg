...
def build_optimizer_legacy(optimizer_config: dict):
    """
    Parsing the optimiser options and parameters
    from config dictionary.
    ...
    if optimizer_config["name"] == "adam":
        return tf.keras.optimizers.legacy.Adam(**optimizer_config["adam"])
    elif optimizer_config["name"] == "sgd":
        return tf.keras.optimizers.legacy.SGD(**optimizer_config["sgd"])
    elif optimizer_config["name"] == "rms":
        return tf.keras.optimizers.legacy.RMSprop(**optimizer_config["rms"])
    else:
        raise ValueError("Unknown optimizer")